...
fun RusksWafersScreen(
    ...
    Scaffold(
        ...
        LazyVerticalGrid(
            ...
            // Chai's Best Friend Banner
            item(span = { GridItemSpan(maxLineSpan) }) {
                Spacer(modifier = Modifier.height(16.dp))
                ChaiBestFriendBanner()
...
@Composable
fun ChaiBestFriendBanner() {
    Card(
        modifier = Modifier.fillMaxWidth(),
        shape = RoundedCornerShape(16.dp),
        colors = CardDefaults.cardColors(containerColor = Color(0xFFE5E7EB).copy(alpha = 0.5f))
    ) {
        Column(
            modifier = Modifier.padding(24.dp)
        ) {
            Text(
                text = "Chai’s Best Friend",
                style = MaterialTheme.typography.titleLarge,
                fontWeight = FontWeight.Bold,
                color = Color.Black
            )
            Text(
                text = "Freshly baked, perfectly crunchy.",
